...
int main(int argc, char *argv[]) {
	...
	do{
		...
		for(int j = 0; j < N_STREAMS ; j++){
			cudaStreamsCreate(&streams[i]);
			handledFinishedStream[i] = false;
		} 
		...
		if (mode == PROGRAM_MODE_STREAMS) {
			for (int i = 0; i < NREQUESTS; i++) {
				...
				cudaMemcpyAsync(gpu_hist1, 0, 256 * sizeof(int),streams[streamIndex]);
				cudaMemcpyAsync(gpu_hist2, 0, 256 * sizeof(int),streams[streamIndex]);
